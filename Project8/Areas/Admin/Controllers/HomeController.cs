...
namespace WebBanSach.Areas.Admin.Controllers
{
    ...
    public class HomeController : Controller
    {
        ...
        {
            var result = new AdminProcess().DeleteUser(id);
            return Json(new
            {
        ...
        [HttpPost]
        public JsonResult UpdateTracking(int id, int tracking)
        {
            try
            {
        ...
        public JsonResult DeleteOrder(int id)
        {
            try
            {
                ...
                var chiTiet = db.ChiTietDDHs.Where(x => x.MaDDH == id).ToList();
                db.ChiTietDDHs.RemoveRange(chiTiet);
                db.DonDatHangs.Remove(order);
                db.SaveChanges();

                return Json(new { success = true, message = "Đã xóa đơn hàng thành công.", id = id });
            }
            catch
            {
