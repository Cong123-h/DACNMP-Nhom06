    ...
    public class AdminProcess
    {
        ...
        public DeleteResult DeleteUser(int id)
        {
            try
            {
                ...
                if (kh == null)
                    return new DeleteResult { Success = false, Message = "Không tìm thấy người dùng." };

                bool hasOrders = db.DonDatHangs.Any(d => d.MaKH == id);
                if (hasOrders)
                {
                ...
                return new DeleteResult { Success = true, Message = "Đã xóa người dùng thành công." };
            }
            catch
            {
                return new DeleteResult { Success = false, Message = "Lỗi hệ thống. Vui lòng thử lại." };
            }
        }
